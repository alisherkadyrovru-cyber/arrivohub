...
export default function RegisterAssistant() {
  ...
  const [form, setForm] = useState({
    ...
    tursabCard: "" as "Yes"|"No"|"", tursabNumber: "",
    phone: "+90 ", email: "", password: "", repeatPassword: "",
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const t = await registerUser({
        ...
        tursabCard: form.tursabCard === "Yes" ? form.tursabNumber : undefined,
      ...
      <div className="mx-auto mt-8 max-w-lg px-4">
        <div className="glass p-6 space-y-5">
          <div className="flex items-center justify-between">
            <div className="text-lg font-semibold">Create Greeter / Assistant Account</div>
          ...
          {token ? (
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div>
                <div className="label mb-2">Do you have a Tursab card? *</div>
                <div className="flex gap-4 text-sm">
                  {(["Yes","No"] as const).map(v => (
                    <label key={v} className="flex items-center gap-2 cursor-pointer">
                      <input type="radio" name="tursab" className="accent-white" checked={form.tursabCard === v} onChange={() => set("tursabCard", v)} />
                      {v}
                    </label>
                  ))}
                </div>
                {form.tursabCard === "Yes" && (
                  <input className="input mt-2" placeholder="Tursab Card Number" value={form.tursabNumber} onChange={e => set("tursabNumber", e.target.value)} />
                )}
              </div>
              <label className="block">
