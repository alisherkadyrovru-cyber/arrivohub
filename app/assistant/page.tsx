...
export default function Feed() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setMe(p);
      const rs = await listRequests({ kind: "assistant", status: "open" } as any);
  ...
  async function apply(requestId: string, offerTry: number | null) {
    if (!me) return;
    try {
      await addApplication(requestId, { applicantId: me.id, applicantName: me.fullName, role: "assistant", offerTry });
  ...
  return (
    <AppShell role="assistant" navItems={assistantNav as any} centerTitle={(p) => p.fullName} rightSlot={(p) => <span className="chip">Sub until: {p.expiresAt ?? "—"}</span>}>
      <div className="space-y-4">
        <div className="text-lg font-semibold">Feed — available requests</div>
